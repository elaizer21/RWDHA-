...
if (keys.length === 0) {
  keys = [
    { id: 'KEY001', num: 'K001', room: 'Main Entrance', floor: 'Ground', category: 'Master', remarks: 'Master Key', status: 'Available' },
    { id: 'KEY002', num: 'K002', room: 'Executive Suite', floor: '5th', category: 'Room', remarks: 'VIP Access', status: 'Available' },
    { id: 'KEY003', num: 'K003', room: 'Storage Room', floor: 'Basement', category: 'Storage', remarks: 'Restricted', status: 'Available' },
    { id: 'KEY004', num: 'K004', room: 'Pool Area', floor: 'Garden', category: 'Emergency', remarks: 'Outdoor', status: 'Available' },
    { id: 'KEY005', num: 'K005', room: 'Spa', floor: '3rd', category: 'Room', remarks: 'Wellness Area', status: 'Available' }
  ];
...
function handleCheckOut(e) {
  ...
  // Validation
  if (!/^[A-Za-z0-9]{3,10}$/.test(data.employee_id)) {
...
function renderCheckinTable() {
  ...
  // Filter records that have at least one item not returned
  const nonReturnedRecords = accessHistory.filter(record => 
    !record.items_returned.every(item => item.returned)
  );
  ...
  filteredRecords.forEach(record => {
    const tr = document.createElement('tr');
    const pendingItems = record.items_returned.filter(item => !item.returned).length;
    const totalItems = record.items_returned.length;
    const status = pendingItems === totalItems 
      ? '<span class="status-badge status-taken">TAKEN</span>' 
      : `<span class="status-badge status-partial">PARTIAL RETURN</span>`;
...
function showCheckinModal(recordId) {
  ...
  // Get pending items
  const pendingItems = record.items_returned.filter(item => !item.returned);
    ...
    <div class="return-section">
      ...
      <div class="returned-items-list">
        ${pendingItems.map(item => {
          ...
          return `
            <div class="return-item item-pending" id="return_item_${item.type}_${item.id}">
              <div class="return-item-info">
                ${icon}${itemDetails}
              </div>
              <div class="return-item-status">
                <input type="checkbox" id="return_${item.type}_${item.id}" class="return-item-checkbox" onchange="toggleReturnItem('${item.type}', '${item.id}', this.checked)">
                <label for="return_${item.type}_${item.id}">Returned</label>
              </div>
            </div>
          `;
        }).join('')}
...
function toggleReturnItem(type, id, isReturned) {
  const itemElement = document.getElementById(`return_item_${type}_${id}`);
  if (isReturned) {
    itemElement.classList.remove('item-pending');
    itemElement.classList.add('item-returned');
  } else {
    itemElement.classList.remove('item-returned');
    itemElement.classList.add('item-pending');
  }
}
...
function handleCheckinSubmit(recordId) {
  ...
  checkboxes.forEach(checkbox => {
    const id = checkbox.id.replace('return_key_', '').replace('return_card_', '');
    const type = checkbox.id.includes('return_key_') ? 'key' : 'card';
    returnedItems.push({ type, id });
  });
  ...
  // Update the record: mark these items as returned
  returnedItems.forEach(returnedItem => {
    const item = record.items_returned.find(item => 
      item.type === returnedItem.type && item.id === returnedItem.id
    );
    if (item) {
      item.returned = true;
    }
  ...
  // If all items are now returned, set the time_brought
  const allReturned = record.items_returned.every(item => item.returned);
  if (allReturned) {
...
function renderReport() {
  ...
  pageData.forEach(h => {
    const allReturned = h.items_returned.every(item => item.returned);
    const anyPending = h.items_returned.some(item => !item.returned);
    
    let status = allReturned 
      ? '<span class="status-badge status-success">RETURNED</span>' 
      : anyPending 
        ? (isOver12Hours(h.time_taken) 
          ? '<span class="status-badge status-overdue">OVERDUE</span>' 
          : '<span class="status-badge status-taken">TAKEN</span>')
        : '';
    
    let tr = document.createElement('tr');
    if (!allReturned && isOver12Hours(h.time_taken)) tr.classList.add('unreturned');
    
    tr.innerHTML = `
      <td>${h.key_taken ? `${h.key_taken} (x${h.key_quantity})` : ''}</td>
      <td>${keys.find(k => k.id === h.key_id)?.room || ''}</td>
      <td>${h.name||''}</td>
      <td>${h.issued_by||''}</td>
      <td>${h.time_taken||''}</td>
      <td>${status}</td>
...
function renderEmployeeTable() {
  ...
  pageData.forEach(emp => {
    ...
    tr.innerHTML = `<td class="formatted-number">${emp.id}</td>
      ...
      <td>${emp.position||''}</td>
      <td class="flex gap-1">
        <button onclick="editEmployee('${emp.id}')" class="rw-btn edit-btn text-xs px-2 py-1 rounded">Edit</button>
...
function addKey(e) {
  ...
  keys.push({ id: 'KEY' + Date.now().toString().slice(-6), num, room, floor, category, remarks, status: 'Available' });
...
function renderKeyTable() {
  ...
  pageData.forEach(k => {
    ...
    tr.innerHTML = `<td class="formatted-number">${k.num}</td>
      ...
      <td>${status}</td>
      <td class="flex gap-1">
        <button onclick="editKey('${k.id}')" class="rw-btn edit-btn text-xs px-2 py-1 rounded">Edit</button>
...
function filterKeyTable() {
  keySearchTerm = document.getElementById('keySearch').value;
  keyCurrentPage = 1;
  renderKeyTable();
}
...
function exportKeys() {
  const headers = ['num', 'room', 'category', 'floor', 'remarks'];
  const csv = [headers.join(',')];
  keys.forEach(k => {
    csv.push([k.num, k.room || '', k.category || '', k.floor || '', k.remarks || ''].map(field => `"${field}"`).join(','));
  });
  const csvStr = csv.join('\n');
  const blob = new Blob([csvStr], { type: 'text/csv;charset=utf-8;' });
  const link = document.createElement('a');
...
function importKeys(files) {
  ...
  reader.onload = function(e) {
    ...
    rows.forEach(row => {
      ...
      const [num, room, category, floor, remarks] = cols;
      if (!num) return;
      // Check if already exists
      if (!keys.some(k => k.num === num)) {
        newKeys.push({ id: 'KEY' + Date.now().toString().slice(-6), num, room, category, floor, remarks, status: 'Available' });
      }
    });
    keys = [...keys, ...newKeys];
...
function renderCardTable() {
  ...
  pageData.forEach(c => {
    ...
    tr.innerHTML = `<td class="formatted-number">${c.number}</td>
      ...
      <td>${status}</td>
      <td class="flex gap-1">
        <button onclick="editCard('${c.id}')" class="rw-btn edit-btn text-xs px-2 py-1 rounded">Edit</button>
...
function filterCardTable() {
  cardSearchTerm = document.getElementById('cardSearch').value;
  cardCurrentPage = 1;
  renderCardTable();
}
